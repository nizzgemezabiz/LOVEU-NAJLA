/**
 * MASTER INTERACTIVE CONTROLLER — WEB SCRAPBOOK NAJLA PREMIUM
 * Optimized Version: Unified Canvas Pipeline, Strict 3:4 Aspect Protection,
 * and Reactive LocalStorage Engine.
 */
...
let particlesArray = [];
let confettiArray = []; // Unified array to prevent canvas rendering race conditions
...
const romanticQuotes = [
    ...
    "Kamu selalu memiliki cara terseni untuk membuat hal biasa terasa sangat istimewa.",
...
function setupMathGame() {
    ...
    document.getElementById('mathQuestion').innerText = questionText;
}
...
document.getElementById('btnVerify').addEventListener('click', () => {
    ...
    if (userAns === currentMathAnswer) {
        ...
        startBtn.classList.add('unlocked-now');
        startBtn.querySelector('span').innerText = "Buka Scrapbook";
        
        const lockIcon = startBtn.querySelector('.lock-icon');
        if (lockIcon) lockIcon.remove();
        document.getElementById('mathAnswer').blur();
    } else {
...
// ==========================================================================
// 2. ROMANTIC TEXT RANDOM INJECTION ROUTINES
// ==========================================================================
function setupTextInjections() {
    ...
    document.getElementById('finalClosingPhrase').innerText = pool[15];
}
...
// ==========================================================================
// 3. INTERACTIVE CLICK EVENTS MANAGEMENT
// ==========================================================================
function setupInteractiveEvents() {
    ...
    cards.forEach(card => {
        card.addEventListener('click', () => {
            const imgSrc = card.querySelector('.scrapbook-img').getAttribute('src');
    ...
    document.getElementById('btnSubmitWish').addEventListener('click', () => {
...
function attemptAutoplay() {
    ...
    audio.play().then(() => {
        ...
        statusText.innerText = "Tap Play";
    });
}

// ==========================================================================
// 5. MOTION GRAPHICS & SCROLL MAP OBSERVERS
// ==========================================================================
            ...
            window.requestAnimationFrame(() => {
                ...
                if (closingSec) {
                    ...
                    if (rect.top <= window.innerHeight * 0.6) {
                        closingSec.classList.add('active-night');
                    } else {
                        closingSec.classList.remove('active-night');
                    }
                }
                ticking = false;
            });
...
// ==========================================================================
// 6. FLOATING PARTICLES & UNIFIED CANVAS ENGINE (Anti-Flicker)
// ==========================================================================
function setupCanvasParticles() {
    ...
    // Unified 60 FPS Animation Engine Loop
    function animate() {
        ctx.clearRect(0, 0, canvas.width, canvas.height);
        
        // 1. Render Floating Background Particles
        particlesArray.forEach(p => {
            p.update();
            p.draw();
        });
        
        // 2. Render Confetti Burst Particles Smoothly without Conflict
        for (let j = confettiArray.length - 1; j >= 0; j--) {
            let cp = confettiArray[j];
            cp.y += cp.speedY;
            cp.x += cp.speedX;
            cp.rotation += cp.rotationSpeed;
            cp.opacity -= 0.006; // Elegant continuous fade out
...
// Procedural Safe Confetti Injector Pipeline
